...
#include <omp.h>
  ...
  int getNeighbors(int** grid, int i, int j); //Retorna os vizinhos de cada c̩lula
  int **grid;
  int **newGrid; 
  int **aux;
  int QUANTIDADE_THREADS = 4;
  ...
  void atualizaGrid(){
    int companheiro, i, j;
    #pragma omp parallel private(i, j, companheiro) shared(grid, newGrid)
    {
...
void passarGeracao(){
  ...
  for(int cont = 0; cont < GERACAO_TOTAL; cont++){
    atualizaGrid();
    aux = grid;
    grid = newGrid;
    newGrid = aux;
