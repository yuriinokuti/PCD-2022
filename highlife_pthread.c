  ...
  void *atualizaGrid(void *quant){
    int companheiro;
    int quantidade = (int)quant;
    for(int i = quantidade; i < TAMANHO_TOTAL; i = i+tt){
      for(int j=0; j < TAMANHO_TOTAL; j++){
        ...
        if(companheiro == 2 && grid[i][j] == 1){
          ...
          if(companheiro == 3 || companheiro == 6){
            newGrid[i][j] = 1;
          }
          else{
            newGrid[i][j] = 0;
  ...
  void passarGeracao(){
    ...
    for(int cont = 0; cont < 2000; cont++){
      for(int quant = 0; quant<tt;quant++){
        create = pthread_create(&t1[quant], NULL, atualizaGrid,(void*)quant); //criando
      }
    
      for(int quant = 0; quant<tt;quant++){
        pthread_join(t1[quant],NULL);
      }
